...
class _UploadScreenState extends State<UploadScreen> {
  ...
  _uploadItemImageToStorage(image)async{
    if(_file != null){
      try{
        Reference ref =  FirebaseStorage.instance.ref().child('SingleItemImages').child(_auth.currentUser!.uid);
  ...
  Future <List<String>?> _uploadMultipleItemImageToStorage()async{
    ...
    if(_pickFileList.isNotEmpty){
      try{
        for(PlatformFile file in _pickFileList){
          Reference ref =  FirebaseStorage.instance.ref().child('imageMulti').child(FirebaseAuth.instance.currentUser!.uid);
    ...
    setState(() {
      ...
      await productCollection.doc(_itemId).set({
        'FullName': _creatorData['FullName'],
        'Headers': _creatorData['Headers'],
        'ID': _creatorData['ID'],
        'ProfilePic':_creatorData['ProfilePic'],
        'contact': _creatorData['contact'],
        'email': _creatorData['email'],
        'like': _creatorData['like'],
        'links': _creatorData['links'],
        'location': _creatorData['location'],
